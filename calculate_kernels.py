...
if __name__ == '__main__':
    ...
    n_gpus = 1
    output_path = '/scratch/etv21/conv_gp_data/exp4/exp4_E2'
    data_path = '/scratch/etv21/conv_gp_data/MNIST_data/eps=0.3_arch_{}'
    #Relative to the data path
    adv_data_file = '../reverse/gp_adversarial_examples_noisy_eps=0.3_norm_2.npy' #'two_vs_seven_adversarial.npy' # 
    param_constructor = SimpleMNISTParams

    for arch_num in range(0,1):
        
        data_path_a = data_path.format(arch_num)
        print('Data path: ' + data_path_a)
        ...
        kernel_path = os.path.join(output_path, "kernels")
        if not os.path.exists(kernel_path):
            os.makedirs(kernel_path)
            print("Directory " , kernel_path ,  " Created ")


        files = dict(
            params = os.path.join(output_path, "params_{:02d}_arch_{}.pkl.gz".format(seed, arch_num)),
            Kv_diag = os.path.join(kernel_path, "Kv_diag{:02d}.npy".format(seed)),
            Kxvx = os.path.join(kernel_path, "Kxvx_{:02d}.npy".format(seed)),
            Kt_diag = os.path.join(kernel_path, "Kt_diag{:02d}.npy".format(seed)),
            Kxtx = os.path.join(kernel_path, "Kxtx_{:02d}.npy".format(seed)),
            Ka_diag = os.path.join(kernel_path, "Ka_diag{:02d}_arch_{}.npy".format(seed, arch_num)),
            Kxax = os.path.join(kernel_path, "Kxax_{:02d}_arch_{}.npy".format(seed, arch_num)),
            Kxx = os.path.join(kernel_path, "Kxx_{:02d}.npy".format(seed)),
        )
        ...
        print("Params:", params)
        pu.dump(params, files['params'])
        with tf.device("GPU:0"):
            kern = create_kern(params)

        save_kernels(kern, n_gpus, files, data_path_a, adv_data_file)
'''
